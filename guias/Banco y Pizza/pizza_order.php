  ...
  <div>
    ...
    <form method="POST" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>" novalidate>
      ...
      <div class="card" style="margin-top:20px">
        ...
        ?>
        <label class="pizza-card <?= $sel ? 'selected' : '' ?>" id="wrap-<?= $pizza->getId() ?>">
          <input type="checkbox" name="pizzas[]" value="<?= $pizza->getId() ?>"
                 <?= $sel ? 'checked' : '' ?> onchange="toggleCard(this)">
          <div class="pizza-chk">
          ...
          <span class="pizza-right"><?= $pizza->getFormattedPrice() ?></span>
        </label>
        <?php endforeach; ?>
...
function toggleCard(cb) {
    cb.closest('.pizza-card').classList.toggle('selected', cb.checked);
    updateSummary();
}
